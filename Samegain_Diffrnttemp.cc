...
SameGain_DiffrntTemp::SameGain_DiffrntTemp(int nFiles)
{
    ...
    g_temp_meanLY = new TGraphErrors(nFiles);
}
...
{
    calculate_ly_against_temp_Results();

    //draw_meanGain_against_temperature();
    //draw_voltage_against_temperature();
    //draw_GainAndVoltage_against_temp();

    //draw_gain_temperature_distributions();

    //draw_ly_temperature_distributions();
}

void SameGain_DiffrntTemp::calculate_ly_against_temp_Results()
{
    /////
    correct_ly_by_gain();
    /////
    ...
    for (unsigned int i=0; i<temperature_meanLY_histogram_pair.size(); ++i)
    {
        ...
        double meanLY_err = HISTOGRAM_FUNCTION::cropped_getMeanValueError(temperature_meanLY_histogram_pair.at(i).second);
        g_temp_meanLY->SetPoint(i, temperature, meanLY);
        g_temp_meanLY->SetPointError(i, meanLY_err/*insert temp err*/, meanLY_err);
    }
    TCanvas* c6 = new TCanvas("c6", "c6");
    c6->cd();
    g_temp_meanLY->SetTitle("");
    g_temp_meanLY->Draw("ape");
    TF1 *linFit = new TF1 ("linFit", "[0]+x*[1]", -300., 50.);
    linFit->SetParNames("Offset", "Slope");
    g_temp_meanLY->Fit(linFit);

    std::string outfilePath = analysis_result_DIR + std::string("/meanLY_temperature_graph.pdf");
    c6->SaveAs(outfilePath.c_str(), "Overwrite");
    c6->Close();
    delete c6;
}

void SameGain_DiffrntTemp::draw_ly_temperature_distributions()
{
    ...
    t4->Draw();
    std::string outfilePath = analysis_result_DIR + std::string("/LY_temperature_distributions.pdf");
...
void SameGain_DiffrntTemp::draw_gain_temperature_distributions()
{
    ...
    t3->Draw();
    std::string outfilePath = analysis_result_DIR + std::string("/gain_temperature_distributions.pdf");
    c3->SaveAs(outfilePath.c_str(), "Overwrite");
    c3->Close();
    delete c3;
}

void SameGain_DiffrntTemp::draw_voltage_against_temperature()
{
    ...
    std::string outfilePath = analysis_result_DIR + std::string("/voltage_against_temperature.pdf");
    c2->SaveAs(outfilePath.c_str(), "Overwrite");
    c2->Close();
    delete c2;
}

void SameGain_DiffrntTemp::draw_meanGain_against_temperature()
{
    for (unsigned int i=0; i<temperature_gain_histogram_pair.size(); ++i)
    {
        double temperature = temperature_gain_histogram_pair.at(i).first;
...
void SameGain_DiffrntTemp::correct_ly_by_gain()
{
    TH1D* h_ref = 0;
    for (unsigned int i=0; i<temperature_gain_histogram_pair.size(); ++i)
    {
        double temperature = temperature_gain_histogram_pair.at(i).first;
        if  (temperature > 19.9 && temperature < 20.1)
            h_ref = temperature_gain_histogram_pair.at(i).second;
    }

    for (unsigned int i=0; i<temperature_meanLY_histogram_pair.size(); ++i)
    {
        assert(temperature_meanLY_histogram_pair.at(i).first == temperature_gain_histogram_pair.at(i).first);

        TH1D* h_temp = temperature_gain_histogram_pair.at(i).second;
        assert (h_ref->GetNbinsX() == h_temp->GetNbinsX());
        for( int b=1; b<=h_ref->GetNbinsX(); ++b )
        {
            double gain_ref = h_ref->GetBinContent(b);
            double gain_temp = h_temp->GetBinContent(b);
            //std::cout << gain_temp/gain_ref << std::endl;
            double corrected_channel_lightYield = temperature_meanLY_histogram_pair.at(i).second->GetBinContent(b) * gain_ref/gain_temp;
            temperature_meanLY_histogram_pair.at(i).second->SetBinContent(b, corrected_channel_lightYield);
        }
    }
}
